...
const willTransitionTo = (nextState, replaceState, callback, appInit=true) => { //appInit is true when called via router onEnter, and false when node is manually selected in access settings
    ...
    if (connect) ss.set("latencyChecks", latencyChecks + 1); // Every 5 connect attempts we refresh the api latency list
    if (latencyChecks >= 5) {
        apiLatenciesCount = 0;
    ...
    */
    ...
    var onConnect = () => {
        ...
        if (Apis.instance()) {
            ...
            if (!autoSelection) {
                SettingsActions.changeSetting({setting: "apiServer", value: currentUrl});
            }
