...
const AccountOrders = ({assets, account}) => {

    const _cancelLimitOrder = (id, e) => {
        e.preventDefault();
    ...
    const cancel = counterpart.translate("account.perm.cancel");
    let markets = {};

    let marketOrders ={};

    if (!account || !account.get("orders")) {
        return null;
    }
        ...
        if (base && quote) {
            ...
            marketOrders[marketID].push(
                <OrderRow
                    ...
                    showSymbols={false}
                    invert={true}
                    onCancel={_cancelLimitOrder.bind(null, order.id)}
                    price={price.full}
                />
            );
        }
