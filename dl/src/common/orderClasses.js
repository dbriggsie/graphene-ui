...
class Asset {
    ...
    getAmount({real} = {}) {
        if (real) {
            return limitByPrecision(this.amount / this.toSats(), this.precision);
        } else {
            return this.amount;
        }
    }
...
class Price {
    constructor({base, quote, real = false} = {}) {
        ...
        if (real && typeof real === "number") {
            ...
            * larger than 10k do not need more than 5 decimals. Without this we
    ...
    toReal(price = this) {
        let real = (price.base.amount * price.quote.toSats()) / (price.quote.amount * price.base.toSats());
        return parseFloat(real.toFixed(8)); // toFixed and parseFloat helps avoid floating point errors for really big or small numbers
    }
}
...
class LimitOrder {
    ...
    setExpiration(expiration = new Date()) {
